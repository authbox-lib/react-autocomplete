...
class Autocomplete extends React.Component {
  ...
  maybeScrollItemIntoView() {
    if (this.isOpen() && this.state.highlightedIndex !== null) {
      ...
      const itemDomNode = itemNode && findDOMNode(itemNode)
      const menuDomNode = menuNode && findDOMNode(menuNode)
  ...
  handleCompletion(event) {
    ...
    if (!this.isOpen()) {
      ...
      this.setState({
        ...
        this.props.onSelect(value, item, nEvent)
      })
    }
  }
  ...
  maybeAutoCompleteText(state, props) {
    if (!this.props.autocompleteInputValue)
  ...
  selectItemFromMouse(item, evt) {
    const nEvent = evt.nativeEvent;
    const value = this.props.getItemValue(item)
  ...
  renderMenu() {
    const items = this.getFilteredItems(this.props).map((item, index) => {
      ...
      return React.cloneElement(element, {
        ...
        onClick: evt => this.selectItemFromMouse(item, evt),
